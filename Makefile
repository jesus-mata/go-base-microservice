...
build: ## Build your project and put the output binary in _out/
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 $(GOCMD) build -a -ldflags ' -w -s  -X"main.version=${VERSION}" -X "main.buildDateTime=${DATE}" -X "main.gitRev=${GIT_REV}"' -o _out/$(BINARY_NAME) ./${BINARY_PATH}
